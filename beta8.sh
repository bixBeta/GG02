...
trimSE(){
        ...
        for i in fastqs/*.gz
        do
            /workdir/TREx_shared/projects/TREX_rna_1.sif trim_galore -j 8  --nextseq 20 --gzip --length 50 $i
        done
...
trimHiSeq(){
        ...
        for i in fastqs/*.gz
        do
            /workdir/TREx_shared/projects/TREX_rna_1.sif trim_galore -j 8  --quality 20 --gzip --length 50 $i
        done
        mv *_trimming_report.txt TrimQC_stats
        mv *trimmed.fq.gz trimmed_fastqs
...
trimPE(){
                ...
                for i in "${fastqs[@]}"
                do
                /workdir/TREx_shared/projects/TREX_rna_1.sif trim_galore -j 8  --nextseq 20 --gzip --length 50  --paired $i
                done
...
trimMiSeqPE(){
                ...
                for i in "${fastqs[@]}"
                do
                  /workdir/TREx_shared/projects/TREX_rna_1.sif trim_galore -j 8  --quality 20 --gzip --length 50  --paired $i
                done
...
align(){
    ...
    for i in *_trimmed.fq.gz
        ...
        /workdir/TREx_shared/projects/TREX_rna_1.sif STAR \
        --runThreadN 12 \
...
geneBodyCov(){
        ...
        echo
        # source /programs/RSeQC2-2.6.1/setup.sh
        /workdir/TREx_shared/projects/TREX_rna_1.sif geneBody_coverage.py -r ${bed12[${DIR}]} -i *.BAMS/ -o ${PIN}
        mkdir geneBodyCov
        mv *geneBodyCoverage.* log.txt geneBodyCov
        cd ..
}
...
#          source /programs/RSeQC2-2.6.1/setup.sh
          ...
          /workdir/TREx_shared/projects/TREX_rna_1.sif geneBody_coverage.py -r ${bed12[${DIR}]} -i chr_${GBCOV}_BAMS -o ${PIN}
...
if [[ -z $1 ]] || [[  $1 = "help"  ]] ; then
    ...
    echo >> beta8.run.log
    echo `date` >> beta8.run.log
    echo "Project Identifier Specified = " $PIN >> beta8.run.log
    echo "Reference Genome Specified   = " $DIR >> beta8.run.log
    echo "Trimming for smRNA seq       = " $T >> beta8.run.log
    echo "SE or PE                     = " $RUN >> beta8.run.log
    echo "Strandedness specified       = " $STRAND >> beta8.run.log
    echo "GeneBody Coverage            = " $GBCOV >> beta8.run.log
    echo >> beta8.run.log

    echo "ENV INFO: " >> beta8.run.log
    echo >> beta8.run.log
